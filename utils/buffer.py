...
class ReplayBuffer(object):
    ...
    def push(self, observations, actions, rewards, next_observations, dones):
        nentries = observations.shape[0]  # handle multiple parallel environments    #Xq observations.shape = threads*agent_num*obsdim
    ...
    def sample(self, N, to_gpu=False, norm_rews=True, act_=False):
        ...
        if act_ == True:
            return ([cast(self.obs_buffs[i][inds]) for i in range(self.num_agents)],
                    [cast(self.ac_buffs[i][inds-1]) for i in range(self.num_agents)],
